...
/**
 * Mix-in class that adds display handling helpers for per-track Kontrol modes.
 *
 * @author Kate Temkin
 */
public interface IKontrolTrackMode extends IKontrolMode {
    ...
    default void updateGraphicsChannelDisplay(IGraphicDisplay display, final int selectedMenu, final boolean isVolume, final boolean isPan) {
        ...
        final ITrackBank tb = this.getModel().getCurrentTrackBank();
        final KontrolProtocolConfiguration config = this.getSurface().getConfiguration();
